...
import FormPengobatan from "../Pengobatan/Form";
import PrimaryButton from "@/Components/PrimaryButton";

export default function DaftarPengobatan({ auth, pengobatan,penyakit, can }) {
    // Mengambil data user dan data pengobatan dari props
    const { crud } = can;
    ...
    // Membuat form untuk menghapus dan mencari data pengobatan
    const {
        data,
        setData,
        get,
    ...
    // Fungsi untuk mencari data pengobatan
    const searchPengobatan = (search) => {
        get(route("Pengobatan.index", { search: search }), {
    ...
    const handleSearch = (e) => {
        ...
        searchPengobatan(e.target.value); // Mencari data pengobatan berdasarkan query pencarian
    };

    // Menyaring data pengobatan berdasarkan input pencarian
    const filteredPengobatan = pengobatan.data;
    ...
    // State untuk menyimpan data pengobatan yang akan dihapus
    const [itemData, setItemData] = useState(null);

    // Fungsi untuk membuka modal konfirmasi penghapusan
    const openModal = (item) => {
        setIsDeleteModalOpen(true);
        setItemData(item); // Mengatur slug pengobatan yang akan dihapus
    };

    // Fungsi untuk menutup modal konfirmasi penghapusan
    const closeModal = () => {
        setIsDeleteModalOpen(false);
        setItemData(null); // Mengatur ulang state slug pengobatan
    };

    // Fungsi untuk menghapus data pengobatan
    const deleteData = (id) => {
        destroy(route("Pengobatan.destroy", { slug: id }), {
    ...
    // State untuk menyimpan data pengobatan yang akan ditambah atau diedit
    const [selectedPengobatan, setSelectedPengobatan] = useState(null);

    // Fungsi untuk membuka modal tambah data
    const openModalAdd = () => {
        setSelectedPengobatan(null);
        setAddModalOpen(true);
    };

    // Fungsi untuk membuka modal edit data
    const openModalForEdit = (pengobatan) => {
        setSelectedPengobatan(pengobatan); // Mengatur data pengobatan yang akan diedit
        setAddModalOpen(true);
    };
        ...
        >
            ...
            <Modal show={isAddModal} onClose={() => setAddModalOpen(false)}>
                ...
                <div className="p-2 block">
                    ...
                    <FormPengobatan
                        auth={auth}
                        pengobatan={selectedPengobatan}
                        setModalOpen={setAddModalOpen}
                        penyakit={penyakit}
                    />
                </div>
            </Modal>
            <Head title="DaftarPengobatan" />
                    ...
                    <div className="bg-white shadow-md sm:rounded-lg p-6">
                        ...
                        <div className="mt-6 overflow-x-auto">
                            <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                                ...
                                >
                                    {filteredPengobatan.length > 0 ? (
                                        filteredPengobatan.map((item, index) => (
                                            <tr
                                                key={item.id}
                                                className="hover:bg-gray-50 transition duration-200"
                                            >
                                                <td className="py-3 px-4 border border-gray-200 text-sm text-gray-700">
                                                    {(pengobatan.current_page - 1) * pengobatan.per_page + index + 1 }
                                                </td>
                                                <td className="py-3 px-4 border border-gray-200 text-sm text-gray-700">
                                                    {item.penyakit.nama}
                                                </td>
                        ...
                        <div className="mt-4 flex justify-center">
                            {pengobatan.links.map((link, index) => (
                                <Link
                                    key={index}
                                    href={link.url}
                                    className={`px-3 py-2 ${
